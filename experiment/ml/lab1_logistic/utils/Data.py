...
class DataSet(object):
    def __init__(self, feature: np.array, label: np.array, title: list, dataClassList: list = None, labelConvert = False):
        '''
    ...
    def getDataIterWithoutBatch(self, is_shuffle = True, is_converted = True):
        ...
        if is_converted == True:
            if self.__isConverted == False:
                self.convertFeature()
                warning("数据未转化，执行默认转化方法。请检查定序数据是否存在问题。")
            return zip(self.__convertedFeature, self.__label)
        else:
